...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function fetchUrlsForDomain(domain) {
            fetch(`/fetch-urls?domain=${encodeURIComponent(selectedText)}`, {
                    headers: {
                ...
                .then(data => {
                    document.querySelector('tbody').innerHTML = data.urls.map(url => `
        <tr>
    ...
    `).join('');
                    ...
                    document.querySelector('.pagination-info').innerHTML = data.pagination;
                ...
                link.addEventListener('click', function(e) {
                    ...
                    fetch(url, {
                        ...
                        .then(data => {
                            document.querySelector('tbody').innerHTML = data.urls.map(url => `
                    <tr>
                        <!-- Same table row structure as above -->
                    </tr>
                `).join('');
